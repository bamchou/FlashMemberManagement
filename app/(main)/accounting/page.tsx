...
const MENU_GROUPS: MenuGroup[] = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        label: '大会参加費管理',
        description: '大会への参加メンバーと参加費を管理します',
        comingSoon: false,
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  },
  {
    ...
    items: [
      ...
      {
        href: '/accounting/dues-frequency',
        label: '練習頻度一括設定',
        description: 'メンバーの練習頻度を一覧から簡単に設定・変更します',
        comingSoon: false,
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {
        href: '/accounting/practice-fees',
        label: '月謝金額設定',
        description: '練習頻度ごとの月謝金額を設定します',
        comingSoon: false,
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    >
      <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
        item.comingSoon
          ...
          : 'bg-[#F5C800]/20 text-[#1A3666] group-hover:bg-[#F5C800]/40'
      }`}>
        {item.icon}
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className={`font-bold ${item.comingSoon ? 'text-gray-500' : 'text-[#1A3666]'}`}>{item.label}</p>
          {item.comingSoon && (
            <span className="text-[10px] font-bold bg-blue-50 text-blue-500 border border-blue-200 px-1.5 py-0.5 rounded-full shrink-0">開発予定</span>
          )}
        </div>
