...
import { updatePracticeFee } from '../actions'
import type { PracticeFeeSetting } from '@/lib/types'

export default function PracticeFeeForm({ setting }: { setting: PracticeFeeSetting }) {
  const [amount, setAmount] = useState(String(setting.monthly_fee))
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await updatePracticeFee(setting.frequency, val)
      if (result.error) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#EAE0A8] p-5">
      <p className="text-sm font-bold text-[#1A3666] mb-1">週{setting.frequency}回</p>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          />
          <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-gray-500">円/月</span>
        </div>
        <button
          type="button"
          onClick={handleSave}
