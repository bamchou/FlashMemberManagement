...
/**
 * その日の体育館予約担当（管理者以外は自分の分だけ）
 * mineCount: 自分の担当枠数 / names: 担当者名 / myGyms: 自分が予約する体育館
 */
export type GymDuty = { mineCount: number; names: string[]; myGyms: string[] }
...
export function GymDutyCellBadge({ date, duty }: { date: string; duty: GymDuty }) {
  ...
  return (
    <Link
      ...
      title={isMine
        ? `あなたが体育館予約の担当です${duty.myGyms.length > 0 ? `（${duty.myGyms.join('、')}）` : ''}`
        : `体育館予約：${duty.names.join('、')}`}
    >
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0">
        <span className="block text-sm font-semibold text-[#1A3666] truncate">
          {isMine
            ? `あなたが予約担当です${duty.mineCount > 1 ? `（${duty.mineCount}件）` : ''}`
            : `担当：${duty.names.join('、')}`}
        </span>
        {isMine && duty.myGyms.length > 0 && (
          <span className="block text-xs text-gray-600 truncate">{duty.myGyms.join('、')}</span>
        )}
      </span>
    </Link>
  )
